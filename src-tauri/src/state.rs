...
pub struct AppState { pub store: ConfigStore, pub configuration: Mutex<AppConfiguration>, pub snapshot: Mutex<UiSnapshot> }

impl AppState {
    pub fn new(store: ConfigStore, configuration: AppConfiguration) -> Self {
        let status = if configuration.selected_sonos_id.is_some() { UiStatus::Connecting } else { UiStatus::ConfigurationRequired };
        Self { store, configuration: Mutex::new(configuration.clone()), snapshot: Mutex::new(UiSnapshot { configuration, status, sonos_name: None, sonos_volume: None, local_volume: None, muted: None }) }
    }
