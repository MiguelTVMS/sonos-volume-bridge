use sonos_volume_bridge_sonos::{parse_device_description, parse_last_change, parse_ssdp_response, EventDeduplicator, SonosError};
use url::Url;
...
fn parses_ssdp_location_only_from_successful_response() {
    ...
    assert_eq!(parse_ssdp_response(response.as_bytes()).unwrap().as_str(), LOCATION);
    assert!(matches!(parse_ssdp_response(b"HTTP/1.1 404 Not Found\r\n\r\n"), Err(SonosError::InvalidSsdpResponse)));
}

#[test]
fn rejects_public_ssdp_location() {
    let response = b"HTTP/1.1 200 OK\r\nLOCATION: http://8.8.8.8/xml/device_description.xml\r\n\r\n";
    assert!(matches!(parse_ssdp_response(response), Err(SonosError::NonLocalHost(_))));
}

#[test]
fn parses_device_description_and_stable_identity() {
    let device = parse_device_description(include_bytes!("fixtures/device-description.xml"), &Url::parse(LOCATION).unwrap()).unwrap();
    assert_eq!(device.id.as_str(), "uuid:RINCON_00000000000101400");
    assert_eq!(device.friendly_name, "Living Room");
    assert_eq!(device.rendering_control.control_url.as_str(), "http://192.168.1.40:1400/MediaRenderer/RenderingControl/Control");
}

#[test]
fn rejects_malformed_device_description() {
    assert!(parse_device_description(include_bytes!("fixtures/malformed.xml"), &Url::parse(LOCATION).unwrap()).is_err());
}
...
fn grouped_metadata_keeps_selected_player_identity() {
    let device = parse_device_description(include_bytes!("fixtures/grouped-device-description.xml"), &Url::parse(LOCATION).unwrap()).unwrap();
...
async fn mock_server_supports_rendering_control_round_trip() {
    let server = MockSonosServer::start(MockSonosState::default()).await.unwrap();
    ...
    let device = SonosDevice {
        ...
        model_name: None,
        model_number: None,
        rendering_control: RenderingControlService { control_url: base.join("control").unwrap(), event_url: base.join("event").unwrap() },
    };
    let client = SonosClient::builder().build().unwrap();
    assert_eq!(client.get_volume(&device).await.unwrap().get(), 20);
    client.set_volume(&device, SonosVolume::new(33).unwrap()).await.unwrap();
    client.set_mute(&device, MuteState(true)).await.unwrap();
    assert_eq!(server.state().await, MockSonosState { volume: SonosVolume::new(33).unwrap(), muted: MuteState(true) });
}
