...
pub enum SonosObservationSource {
    Event,
    ExplicitRead,
    Poll,
}
...
impl Synchronizer {
    ...
    pub const fn confirmed(&self) -> Option<ConfirmedSonosState> {
        self.confirmed
    }
    pub const fn pending_volume(&self) -> Option<PendingIntent> {
        self.pending_volume
    }
    ...
    pub fn handle(&mut self, event: SyncEvent) -> Result<Vec<Effect>, MappingError> {
        match event {
            ...
            SyncEvent::LocalChanged {
                ...
                ..
            } => Ok(vec![Effect::SuppressedLocalCallback]),
            SyncEvent::LocalChanged {
                state,
                origin,
                at_ms,
            } => self.handle_local(state, origin, at_ms),
            SyncEvent::SonosConfirmed {
                volume,
                muted,
                at_ms,
                ..
            } => self.handle_confirmed(volume, muted, at_ms),
        }
    }
    ...
    fn handle_local(
        ...
        _origin: LocalOrigin,
        at_ms: u64,
    ) -> Result<Vec<Effect>, MappingError> {
        let desired = self.mapping.to_sonos(local.volume, self.maximum)?;
        self.pending_volume = Some(PendingIntent::SetVolume {
            desired,
            created_at_ms: at_ms,
        });
    ...
    fn handle_confirmed(
        ...
        self.confirmed = Some(ConfirmedSonosState {
            volume,
            muted,
            revision,
    ...
    pub fn classify_expected_local_callback(
        ...
        expected.classify(observed, now_ms)
    }
}
...
mod tests {
    ...
    fn local(volume: u8) -> LocalAudioState {
        LocalAudioState {
            volume: v(volume),
    ...
    fn startup_confirmation_only_applies_sonos_state() {
        ...
        let effects = machine
            ...
            .unwrap();
        assert_eq!(effects, vec![Effect::ApplyLocal(local(24))]);
        assert_eq!(machine.state(), SyncState::Synchronized);
    }
    #[test]
    fn local_user_write_is_pending_and_capped() {
        let mut machine = sync();
        let effects = machine
            .handle(SyncEvent::LocalChanged {
                state: local(80),
                origin: LocalOrigin::User,
                at_ms: 10,
            })
            .unwrap();
        assert_eq!(
            effects,
            vec![
                Effect::RequestSonosVolume(s(55)),
                Effect::RequestSonosMute(MuteState(false))
            ]
        );
        assert_eq!(
            machine.pending_volume(),
            Some(PendingIntent::SetVolume {
                desired: s(55),
                created_at_ms: 10
            })
        );
    ...
    #[test]
    fn latest_local_intent_replaces_older_one() {
        let mut machine = sync();
        machine
            .handle(SyncEvent::LocalChanged {
                state: local(10),
    ...
    #[test]
    fn external_sonos_confirmation_wins() {
        let mut machine = sync();
        machine
            .handle(SyncEvent::LocalChanged {
                state: local(30),
        ...
        let effects = machine
            ...
            .unwrap();
        assert_eq!(effects, vec![Effect::ApplyLocal(local(35))]);
        assert_eq!(machine.pending_volume(), None);
    }
    #[test]
    fn application_callback_never_writes_sonos() {
        let mut machine = sync();
        let effects = machine
            .handle(SyncEvent::LocalChanged {
                state: local(30),
