...
impl AppConfiguration {
    pub fn validate(&self) -> Result<(), ConfigError> {
        if self.schema_version != SCHEMA_VERSION { return Err(ConfigError::UnsupportedSchema(self.schema_version)); }
        if self.maximum_sonos_volume.get() > 100 { return Err(ConfigError::Invalid("maximumSonosVolume")); }
        self.mapping.validate().map_err(|_| ConfigError::Invalid("mapping"))
    }
}

fn default_points() -> Vec<MappingPoint> {
    [(0, 0), (20, 5), (40, 12), (60, 23), (80, 40), (100, 55)].into_iter().filter_map(|(local, sonos)| Some(MappingPoint { local: sonos_volume_bridge_domain::NormalizedVolume::new(local).ok()?, sonos: SonosVolume::new(sonos).ok()? })).collect()
}
...
impl ConfigStore {
    ...
    pub fn load_or_default(&self) -> Result<AppConfiguration, ConfigError> {
        if !self.path.exists() { return Ok(AppConfiguration::default()); }
        let bytes = fs::read(&self.path)?;
        match serde_json::from_slice::<AppConfiguration>(&bytes).and_then(|configuration| { configuration.validate().map_err(|error| serde_json::Error::io(io::Error::other(error.to_string())))?; Ok(configuration) }) {
            Ok(configuration) => Ok(configuration),
            Err(error) => { self.back_up_corrupt()?; let _ = error; Ok(AppConfiguration::default()) },
        }
    }
    ...
    pub fn reset(&self) -> Result<AppConfiguration, ConfigError> { let configuration = AppConfiguration::default(); self.save(&configuration)?; Ok(configuration) }
    fn back_up_corrupt(&self) -> Result<(), ConfigError> { let backup = self.path.with_extension("json.corrupt"); fs::rename(&self.path, backup)?; Ok(()) }
...
mod tests {
    use super::*;
    #[test] fn default_configuration_is_valid() { AppConfiguration::default().validate().unwrap(); }
}
