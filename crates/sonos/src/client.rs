...
impl SonosClient {
    ...
    async fn soap(
        ...
        let body = format!(
            "<?xml version=\"1.0\" encoding=\"utf-8\"?><s:Envelope xmlns:s=\"http://schemas.xmlsoap.org/soap/envelope/\"><s:Body><u:{action} xmlns:u=\"{RENDERING_CONTROL}\">{arguments}</u:{action}></s:Body></s:Envelope>"
        );
        let response = self
            .http
            .post(endpoint.clone())
            .header("Content-Type", "text/xml; charset=\"utf-8\"")
            .header("SOAPACTION", format!("\"{RENDERING_CONTROL}#{action}\""))
            .body(body)
            .send()
            .await?
            .error_for_status()?;
